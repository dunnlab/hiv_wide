...
sum(hypermut$fisher_p < .05)/nrow(hypermut)
```

## Relaxed clustering criteria
...
cluster_comparison_results <- samples_clustid[[1]] %>%
  # group by threshold
  group_by(threshold) %>%
  nest() %>%
...
```{r}
# 4. --- SUMMARIZE & PLOT: RECOVERY RATES (using Overlap Coefficient) ---
# -----------------------------------------------------------------------
...
total_reference_clusters <- samples_clustid[[1]] %>% select(threshold, id, wgs) %>%
  group_by(threshold) %>%
...
recovery_summary <- imap_dfr(~{
    cluster_comparison_results %>%
      # *** NEW METRIC ***: Filter using overlap_coefficient
      filter(overlap_coefficient > .x) %>%
      group_by(dist_threshold) %>%
      # Count how many *unique* reference clusters were recovered
      summarise(recovered_clusters = n_distinct(ref_cluster_id), .groups = "drop") %>%
      mutate(threshold_label = .y)
    },
    .id = "threshold_value", thresholds
  ) %>%
  # Join with totals to calculate proportion
  left_join(total_reference_clusters, by = "dist_threshold") %>%
  mutate(prop_recovered = recovered_clusters / total_ref_clusters)

comparisons %>% group_by(mix) %>% filter(overlap_coefficient >= .5) %>% summarise(recovered_clusters = n_distinct(wgs_clust_id))
...
ggplot(filter(cluster_comparison_results, overlap_coefficient > 0.5), aes(x=factor(mix), y=overlap_coefficient)) + geom_boxplot() + geom_jitter() + facet_wrap(~threshold)
```
